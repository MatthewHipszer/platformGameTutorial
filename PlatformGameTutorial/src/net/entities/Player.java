...
public class Player {
	...
	public void tick (Block[][] b, ArrayList<MovingBlock> movingBlocks, ArrayList<CollisionLine> cL) {
		...
		int testX = iX + width + (int)GameState.xOffset;
		int testY = iY + (int)GameState.yOffset;
		...
		for (int i = 0; i < b.length; i++) 
			for (int j = 0; j < b[0].length; j++) {
				...
				if (b[i][j].getID() != 0) {			
					...
					//left side
					if (Collision.playerBlock(new Point(testX - width - 1, testY + 2), b[i][j]) || 
							Collision.playerBlock(new Point(testX - width - 1, testY + height - 1), b[i][j])) {
						left = false;
					}
					//top
					if (Collision.playerBlock(new Point(testX - width + 1, testY), b[i][j]) || 
							Collision.playerBlock(new Point(testX - 2, testY), b[i][j])) {
						jumping = false;
						falling = true;
					}
					//bottom
		...
		for (int i = 0; i < movingBlocks.size(); i++) {
			if (movingBlocks.get(i).getID() != 0) {
				if (movingBlocks.get(i).getID() != 0) {			
					//right side
					if (Collision.playerMovingBlock(new Point(testX, testY + 2), movingBlocks.get(i)) || 
							Collision.playerMovingBlock(new Point(testX, testY + height - 1), movingBlocks.get(i))) {
						right = false;
					}
					//left side
					if (Collision.playerMovingBlock(new Point(testX - width - 1, testY + 2), movingBlocks.get(i)) || 
							Collision.playerMovingBlock(new Point(testX - width - 1, testY + height - 1), movingBlocks.get(i))) {
						left = false;
					}
					//top
					if (Collision.playerMovingBlock(new Point(testX - width + 1, testY), movingBlocks.get(i)) || 
							Collision.playerMovingBlock(new Point(testX - 2, testY), movingBlocks.get(i))) {
						jumping = false;
						falling = true;
					}
					//bottom
					if (Collision.playerMovingBlock(new Point(testX - width + 2, testY + height + 1), movingBlocks.get(i)) || 
							Collision.playerMovingBlock(new Point(testX - 2, testY + height + 1), movingBlocks.get(i))) {
						falling = false;
						topCollision = true;
	...
	//find a way to check if the y value of player started above a line and then was below without the x value of the player
	//being greater than the max x value of the line segment or being less than the min x value of the line segment?
		...
		for (int i = 0; i < cL.size(); i++) {
			...
			//right
			if (cL.get(i).intersectsLine(rightX - (width/2), currentY, rightX + 5, currentY) ||
				cL.get(i).intersectsLine(rightX - (width/2), currentY - height, rightX + 5, currentY - height))
			{
			...
			//Top collision
			if (cL.get(i).intersectsLine(leftX + (width/2), prevY - height + 5, 
                                         leftX + (width/2), prevY - height - 5)) {	
			...
			//Bottom collision
			if ((cL.get(i).intersectsLine(leftX + (width/2), prevY - 10, 
					                      leftX + (width/2), prevY + 10)) && 
					                     (!jumping) && (!wallClingRight))                          	
			{
		...
		//Only saves previous y positions when y changes. Same for x
		if (right) {
			GameState.xOffset += moveSpeed;
			moveSpeed = 5;
		}
